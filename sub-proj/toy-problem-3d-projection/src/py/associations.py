...
def img_pt_subtract(pt1, pt2):
	return (pt1.item(0) - pt2.item(0), pt1.item(1) - pt2.item(1) )
...
def knn(proj_pts = [], kdtree = None, k = 1, eps = 0):
	...
	"""
	proj_pts_as_array = np.array([proj_pts]);
	n_proj_pts = proj_pts_as_array.size /2
	proj_pts_as_array = proj_pts_as_array.reshape((1, n_proj_pts, 2))
	(dists_mat, tree_idx_mat) = kdtree.query(proj_pts_as_array, k=k, eps=eps)
